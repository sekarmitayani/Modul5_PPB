...
export default function RecipeDetailPage({ recipeId, onBack }) {
  ...
  const handleSubmitReview = async (e) => {
    ...
    if (result) {
      ...
      // Save local copy of the review so profile can show reviews by username
      try {
        const stored = JSON.parse(localStorage.getItem('user_reviews') || '[]');
        const profile = userService.getUserProfile();
        const localReview = {
          _local_id: `r_${Date.now()}_${Math.random().toString(36).substr(2,6)}`,
          recipeId: recipeId,
        ...
        stored.push(localReview);
        localStorage.setItem('user_reviews', JSON.stringify(stored));
      } catch (err) {
